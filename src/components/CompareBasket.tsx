...
export default function CompareBasket() {
  ...
  return (
    <div className="compare-basket" role="region" aria-label="对比篮">
      <span className="basket-label">
        对比篮 {items.length}/{MAX_COMPARE}
      </span>
      {items.map((x) => (
        <button key={x.id} type="button" className="pill neutral no-dot" onClick={() => remove(x.id)} title="点击移除">
          {x.name} ×
        </button>
      ))}
      {isFull && <span className="pill warn">已达上限</span>}
      <Space size={8}>
