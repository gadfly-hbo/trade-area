...
- 描述列为长文本，内嵌结构化标记，**解析时必须保留、展示时可样式化**：
  ...
  - 项目评级：`【项目评级】X`，森马五级 S/A+/A/B/C（types.ts `ProjectRating`）。md 原无此标记，已由 `scripts/backfill-md.ts` 从 `data/reference/渠道项目管理清单.xlsx`（按商圈名匹配）回填到各 md 的项目性质行；更新渠道清单后重跑该脚本（默认 dry-run，`--apply` 生效，幂等）再 `npm run etl`。指标标签：`【新增指标·xxx】`；字段分隔符：`｜`
  - 核心客群维度含"森马三大人群"预测：A类｜质感流行派 / B类｜都市体面家 / C类｜百搭优选客，带 `±pp` 置信区间
- 省市解析允许「市而无省」（沈阳市…）与直辖市裸名（上海闵行区…）；空省市在 ETL 内用全量「城市→省」共识回填，不引入外部地理数据。
- 客流指标统一为万人次/日：md 口径含「万/日」「万人」「5,500-9,000人次/日」等，裸人次按除以万换算；数字必须带「万/人次/人」单位，无单位不取。
